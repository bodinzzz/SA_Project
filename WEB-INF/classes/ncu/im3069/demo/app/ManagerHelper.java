...
public class ManagerHelper {
	...
	public JSONObject deleteByID(int user_id) {
		...
		try {
			...
			/** SQL指令 */
			String sql = "DELETE FROM `final_project`.`manager` WHERE `user_id` = ? LIMIT 1";
	...
	public JSONObject getAll() {
		...
		try {
			...
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`manager`";
			...
			while (rs.next()) {
				...
				mg = new Manager(user_id, account, user_name, password);
	...
	public JSONObject getByUserID(String user_id) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`manager` WHERE `user_id` = ? LIMIT 1";
			...
			/** 將每一筆會員資料產生一名新Member物件 */
		    mg = new Manager(id, account, user_name, password);
			/** 取出該名會員之資料並封裝至 JSONsonArray 內 */
			jsa.put(mg.getData());
	...
	public JSONObject getByAccount(String account) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`manager` WHERE `account` = ? LIMIT 1";
			...
			int user_id = rs.getInt("user_id");
			String user_name = rs.getString("user_name");
			String this_account = rs.getString("account");
			String password = rs.getString("password");

			
			/** 將每一筆會員資料產生一名新Member物件 */
		    m = new Manager(user_id, this_account, user_name, password);
			/** 取出該名會員之資料並封裝至 JSONsonArray 內 */
			jsa.put(m.getData());
	 ...
	 * @return the JSONObject 回傳SQL指令執行結果與執行之資料
	 */
	public JSONObject updateByUserID(Manager m) {
		/** 紀錄回傳之資料 */
		JSONArray jsa = new JSONArray();
		/** 記錄實際執行之SQL指令 */
		String exexcute_sql = "";
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "Update `final_project`.`manager` SET `account` = ? , `user_name` = ? , `password` = ? WHERE `user_id` = ?";
			/** 取得所需之參數 */
