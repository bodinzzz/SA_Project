...
public class MemberHelper {
	...
	public JSONObject deleteByID(int user_id) {
		...
		try {
			...
			/** SQL指令 */
			String sql = "DELETE FROM `final_project`.`member` WHERE `user_id` = ? LIMIT 1";
	...
	public JSONObject getAll() {
		...
		try {
			...
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`member`";
			...
			while (rs.next()) {
				...
				m = new Member(user_id, user_name, stu_id, email, password, user_phone, created, modified, user_status);
	...
	public JSONObject getByUserID(String user_id) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`member` WHERE `user_id` = ? LIMIT 1";
			...
			/** 將每一筆會員資料產生一名新Member物件 */
		    m = new Member(id, user_name, stu_id, email, password, user_phone, created, modified, user_status);
			/** 取出該名會員之資料並封裝至 JSONsonArray 內 */
			jsa.put(m.getData());
	 ...
	 * @return the JSON object 回傳SQL執行結果與該會員編號之會員資料
	 */
	public JSONObject getByStuID(String stu_id) {
		/** 新建一個 Member 物件之 m 變數，用於紀錄每一位查詢回之會員資料 */
		Member m = null;
		/** 用於儲存所有檢索回之會員，以JSONArray方式儲存 */
		JSONArray jsa = new JSONArray();
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT * FROM `final_project`.`member` WHERE `stu_id` = ? LIMIT 1";
	...
	public boolean checkDuplicate(Member m) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT count(*) FROM `final_project`.`member` WHERE `stu_id` = ?";

			/** 取得所需之參數 */
			String stu_id = m.getStuID();
	...
	public JSONObject create(Member m) {
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "INSERT INTO `final_project`.`member`(`user_name`, `stu_id`, `email`, `password`, `user_phone`, `created`, `modified`, `user_status`)"
					+ " VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
	 ...
	 */
	public JSONObject updateByStuID(Member m) {
		/** 紀錄回傳之資料 */
		JSONArray jsa = new JSONArray();
		/** 記錄實際執行之SQL指令 */
		String exexcute_sql = "";
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "Update `final_project`.`member` SET `user_name` = ? , `email` = ? , `password` = ? , `user_phone` = ?, `modified` = ? WHERE `stu_id` = ?";
			/** 取得所需之參數 */
			String stu_id = m.getStuID();
			String user_name = m.getUserName();
			String email = m.getEmail();
	 ...
	 * @return the JSONObject 回傳SQL指令執行結果與執行之資料
	 */
	public JSONObject updateByUserID(Member m) {
		/** 紀錄回傳之資料 */
		JSONArray jsa = new JSONArray();
		/** 記錄實際執行之SQL指令 */
		String exexcute_sql = "";
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "Update `final_project`.`member` SET `user_name` = ? , `email` = ? , `password` = ? , `user_phone` = ?, `modified` = ? WHERE `user_id` = ?";
			/** 取得所需之參數 */
